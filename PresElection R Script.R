...
Sp.Desc(Pres)
write.csv(Pres, "Pres.csv", row.names=F)
...
write.csv(Pres, "Pres.csv", row.names=F) #Updating Pres CSV file to include winning vote share
...
Pres.Plot <- ggplot(Pres3, aes(x=Year)) +
  ...
  geom_vline(xintercept=seq(1916,2016,4), colour="white") + #white lines to break up the elections
